...
class finances {
	...
	public function insertAcc() {
		...
		$query = sprintf("INSERT INTO %s_finances_accounts (name, description, account_number, iban, swift, institution_name, sort, code, balance, user_id, date, date_update, status)
	...
	public function updateAcc() {
		...
		$query = sprintf("UPDATE %s_finances_accounts
	...
	public function deleteAcc() {
		...
		if($acc->deleteAllAccMovs()) {
			...
			$trash = new trash();
			$trash->setCode(json_encode($acc_obj));
			$trash->setDate();
	...
	public function returnOneAcc() {
		...
		$query = sprintf("SELECT * FROM %s_finances_accounts WHERE id = %s",
				$cfg->db->prefix,
				$this->id
			);
	...
	public function returnAllAccs($where = null, $order = null, $limit = null) {
		...
		$query = sprintf(
			"SELECT * FROM %s_finances_accounts
				WHERE (%s)
				%s %s",
			$cfg->db->prefix,
	...
	public function insertAccMov() {
		...
		$query = sprintf("INSERT INTO %s_finances_accounts_movements (account_id, name, description, value, user_id, date, date_update, status) VALUES (%s, '%s', '%s', %s, %s, '%s', '%s', %s)",
			$cfg->db->prefix,
			$this->acc_id,
			$this->name,
			$this->desc,
	...
	public function updateAccMov() {
		...
		$query = sprintf("UPDATE %s_finances_accounts_movements
	...
	public function deleteAccMov() {
		...
		$acc_mov = new finances();
		$acc_mov->setId($this->id);
		$acc_mov_obj = $acc_mov->returnOneAccMov();

		$trash = new trash();
		$trash->setCode(json_encode($acc_mov_obj));
		$trash->setDate();
		$trash->setModule($cfg->mdl->folder);
		$trash->setUser($authData["id"]);
		...
		if($trash->insert()) {
			$query = sprintf("DELETE
				FROM %s_finances_accounts_movements
				WHERE id = %s",
				$cfg->db->prefix,
				$this->id
			);
	...
	public function returnOneAccMov() {
		...
		$query = sprintf("SELECT * FROM %s_finances_accounts_movements WHERE id = %s",
				$cfg->db->prefix,
				$this->id
			);
	...
	public function returnAllAccMovs($where = null, $order = null, $limit = null) {
		...
		$query = sprintf(
			"SELECT * FROM %s_finances_accounts_movements
				WHERE (%s)
				%s %s",
			$cfg->db->prefix,
			(!empty($where)) ? $where : "true",
	...
	public function refreshAccBalance() {
		...
		$updateQuery = sprintf("UPDATE %s_finances_accounts
			SET balance = %s
			WHERE id = %s",
			$cfg->db->prefix,
			$balance,
